#======================================================================
#
# Python Module to handle an VL53L0X Time of Flight distance sensor 
...
turnleft = 150      # distance from wall to turn left
turnright = 120     # and right
hardturnleft = 220  # close to wall so hard turn left
hardturnright = 80  # and left
sensordeltaforturn = 5 # turn if difference between sensors is greater (mm)
mediumspeed = 40    # slow speed for medium turn was 15
fastspeed = 50      # slow speed for fast turn   was 20
speed = 90          # normal speed
spinSpeed = 70      # speed to spin when approaching wall
speedLeft = speed   # set speed of left wheel in normal use
speedRight = speed  # and right wheel
mediumspeedLeft = 25 # not used
mediumspeedRight = 25 # not used
fastspeedLeft = 40  # not used
fastspeedRight = 40 # not used
rightwheel = 1      # wheel number for setMotor function
leftwheel = 0
frontturndistance =  250    # maze walls are 360 mm apart so aim for middle 210
stopTurningDistance = 200   # stop spining in frontof wheel 
sideDistanceToSwopSensors = 300 # when we get the wall on the other side
...
time.sleep(0.50)
tofLeft.start_ranging(VL53L0X.VL53L0X_BETTER_ACCURACY_MODE)
...
try:
    while distanceSide <= sideDistanceToSwopSensors:   # first runs are turn right using left hand sensors
...
#        pz.setMotor(rightwheel, speedRight)  # set speed back to default. will be overwritten if need for turn     
        ...
        if (distanceFront <= frontturndistance):    # about to hit corner so turn right
            distanceFront = tofFront.get_distance() # second reading in case of one false
            if (distanceFront <= frontturndistance): # really really near corner
                pz.spinRight(spinSpeed)             # spin until certain angle to wall
#            pz.setMotor(leftwheel, speedLeft)
#            pz.setMotor(rightwheel, speedRight - cornerSpeed)
                ...
                while (distanceFront <= stopTurningDistance):   # keep turning until certain angle to front wall                    distanceFront = tofFront.get_distance() 
                    time.sleep(interval)
                ...
                while (distanceSide <= lastDistanceSide):   # keep turning until parallel (least distance) reading from side wall
                    time.sleep(interval)
                    lastDistanceSide = distanceSide
                    distanceSide = tofLeft.get_distance() # get distance from VL53
                    print("Corner turning still ...", distanceSide, lastDistanceSide)

                pz.setMotor(leftwheel, speedLeft)   # start forward again
    ...
    while True:   # remaining runs are all left turns
...
#        pz.setMotor(rightwheel, speedRight)
        ...
        lastDistanceSide = distanceSide     # set distance for checking movement towards/from wall
    ...
    pz.cleanup()    # cleanup piconzero then turn off ToF
